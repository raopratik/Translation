...
class Translation(data.Dataset):
    ...
    def __len__(self):
        return len(self.input_dict["hrl"]['input_ids'])

    def __getitem__(self, index):
        hrl_src = np.array(self.input_dict["hrl"]['input_ids'][index])
        lrl_src = np.array(self.input_dict["lrl"]['input_ids'][index])
        hrl_att = np.array(self.input_dict["hrl"]['attention_mask'][index])
        lrl_att = np.array(self.input_dict["lrl"]['attention_mask'][index])
